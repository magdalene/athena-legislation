...
class SandySpringsCityCouncilMeetingSpider(CrawlSpider):
    ...
    def parse_meeting(self, response):
        ...
        date_text = meeting_info_pars[2].extract().strip()
        time_text = meeting_info_pars[3].extract().strip()
        address = meeting_info_pars[4].extract().strip()
        ...
        for meeting_item_el in meeting_item_contents:
            if not (len(meeting_item_el.xpath(".//span[contains(@class, 'item_list_number')]"))
                    and len(meeting_item_el.xpath(".//div[contaions(@class, 'item_content')]"))):
                continue
            number = meeting_item_el.xpath(".//p[contains(@class, 'meeting_manager')]/text()").extract()[0].strip()
            ...
            link_el = meeting_item_el.xpath(".//div[contains(@class, 'item_content')]/a")[0]
